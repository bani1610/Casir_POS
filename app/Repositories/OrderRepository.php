...
class OrderRepository implements OrderRepositoryInterface
{
    ...
    public function paginate(PaginationDTO $pagination, array $filters = []): LengthAwarePaginator
    {
        return $this->model->newQuery()
            ->with(['user', 'paymentMethod', 'orderItems.menu'])
            ->when($pagination->search, function ($query, $search) {
                $query->where(function ($searchQuery) use ($search) {
                    $searchQuery->where('table_number', 'like', "%{$search}%")
                        ->orWhere('customer_identifier', 'like', "%{$search}%")
    ...
    {
        return $this->model->with(['user', 'paymentMethod', 'orderItems.menu'])->find($id);
    }
    ...
    public function create(OrderDTO $dto): Order
    {
        ...
        return $order->fresh(['user', 'paymentMethod', 'orderItems.menu']);
    }
